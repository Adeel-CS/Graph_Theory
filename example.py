...
for section, content in sections.items():
    ...
    for i in range(len(words)):
        # Add node for each word in the section
        node_label = f"{section}_{words[i]}"  # Label format: section_word
        graph.add_node(node_label)
        if i > 0:
            # Add directed edge between consecutive words in the same section
            source_node = f"{section}_{words[i-1]}"
            target_node = f"{section}_{words[i]}"
            graph.add_edge(source_node, target_node, label=section)
